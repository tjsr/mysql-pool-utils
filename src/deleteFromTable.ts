...
export const deleteFromTable = (table: string, whereClause: any, inputConnection?: Promise<Connection>): Promise<number> => {
  ...
  return mysqlExecute(`DELETE FROM \`${table}\` WHERE ?`, whereClause, inputConnection).then((results: [QueryResult, FieldPacket[]]) => {
